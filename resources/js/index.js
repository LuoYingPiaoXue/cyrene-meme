...
$(function () {
  ...
  const texts = {
    en: {
      ...
      titleHover: "SEE YOU AGAIN, Cyrene",
      ...
      sec: "sec",
      countingUp: "♪",
      footer: "© All rights reserved by miHoYo | Other properties and any right, title, and interest thereof and therein (intellectual property rights included) not derived from Honkai: Star Rail belong to their respective owners.",
    },
    zh: {
      title: "SEE YOU AGAIN, PHILIA093",
      titleHover: "SEE YOU AGAIN, 昔涟",
      subTitle1: "「这次，与我一起……」",
      ...
      sec: "sec",
      countingUp: "♪",
      footer: "© MiHoYo 版权所有 | 其他财产及其相关的任何权利、所有权和利益（包括知识产权）若非源自《崩坏：星穹铁道》，则归其各自所有者所有。",
    },
  };
  ...
  function setLangText(lang = currentLang) {
    ...
    const currentText = $titleElem.find(".title-text").text() || $titleElem.text();
    
    // 如果标题还没有包装，先包装起来
    if ($titleElem.find(".title-text").length === 0) {
      $titleElem.html(`<span class="title-text fade-in">${t.title}</span>`);
    } else {
      $titleElem.find(".title-text").text(t.title);
    }
  ...
  $("#switch-zh").on("click", function () {
    setLangText("zh");
  });

  // Title hover effect for PHILIA093 -> Cyrene/昔涟
  const $title = $(".title");
  let isHovering = false;
  let animationTimeout = null;
  
  function switchTitleText(newText) {
    ...
    // 淡出
    $titleText.removeClass("fade-in").addClass("fade-out");
    
    // 等待淡出完成后切换文字并淡入
    animationTimeout = setTimeout(() => {
      $titleText.text(newText);
      $titleText.removeClass("fade-out").addClass("fade-in");
    }, 250); // 匹配 CSS transition 的一半时间
  }
  
  $title.on("mouseenter", function () {
    isHovering = true;
    const t = texts[currentLang];
    switchTitleText(t.titleHover);
  });
  
  $title.on("mouseleave", function () {
    isHovering = false;
    const t = texts[currentLang];
    switchTitleText(t.title);
  });
